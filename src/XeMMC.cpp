...
uint32_t xbox_emmc_get_ints() { return XSPIReadWORD_(0x30); }
...
bool send_cmd17(unsigned int lba) {
  ...
  // int elapsed = 0;
  // while ((XSPIReadWORD_(0x30) & 0x21) != 0x21) { // Wait for both bits 0 and
  // 5 	Sleep(1); 	if (++elapsed >= timeout) { 		printf("Timeout
  // waiting for 0x21 in
  // 0x30\n"); 		return false;
  //	}
  //}
...
bool send_cmd24(unsigned int lba) {
  ...
  XSPIQueueBytes_(0x0C,
                  0x183A0000); // CMD24, argument/flags may differ for your HW
