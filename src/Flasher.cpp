...
void FlashDataDeInit() { XSPILeaveFlashMode(); }

void FlashDataErase(unsigned int reg) { gGlobalStatus = XNANDErase(reg); }
...
void FlashDataRead(unsigned char *pData, unsigned int Block,
                   unsigned int Words) {
  gGlobalStatus = 0;
  gWORDsLeft = 0;
  gNextBlock = Block;
...
void FlashDataReadBatch(unsigned char *pData, unsigned int Block,
                        unsigned int nPages, unsigned int bytesPerPage) {
  gGlobalStatus = 0;

  XNANDReadBatch(pData, Block, nPages, bytesPerPage / 4, NAND_READ_DELAY_BYTES);
}

void FlashDataWrite(unsigned char Data[], unsigned int Block,
                    unsigned int Words) {
