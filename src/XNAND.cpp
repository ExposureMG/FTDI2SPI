...
extern void GetDataFromDevice(unsigned int dwNumDataBitsToRead,
...
unsigned int XNANDGetStatus() { return XSPIReadWORD_(0x04); }
...
// which cost a full USB read round-trip per page. numBytes * 8 clocks are
// issued at the SPI clock (~0.267 us/byte at 30 MHz). CS is high here, so the
// dummy bytes are ignored by the flash.
static void XNANDQueueReadDelay(unsigned int numBytes) {
  if (numBytes == 0)
    return;

  unsigned int n = numBytes - 1; // MPSSE length field is (count - 1)
  AddByteToOutputBuffer((BYTE)0x19,
                        false); // CLK_DATA_BYTES_OUT, -ve edge, LSB first
...
// Read nPages consecutive physical pages in a single USB write + single USB
// read. Mirrors XNANDReadStart()+XNANDReadProcess() per page, but: status is
// cleared once for the whole batch, the busy-poll is replaced by an in-stream
// delay, and all the page commands/reads are queued before a single
// SendBytesToDevice()/GetDataFromDevice().
//   wordsPerPage : bytes-per-page / 4 (132 for 0x210 incl. spare, 128 for 0x200
//   raw) delayBytes   : page-load delay, see XNANDQueueReadDelay()
