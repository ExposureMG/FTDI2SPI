...
#define PRINT_PROGRESS                                                         \
  printf("\r%0.1f%%", ((block_flash - StartBlock) * 100) /                     \
                          (float)(block_flash_max - StartBlock))
...
extern "C" {
__declspec(dllexport) int spi(int mode, int size, char *file, int startblock,
                              int length) {
  ...
  if (length > 0)
    block_flash_max = StartBlock + (length * 32);
  ...
  if (EraseEnable) {
    ...
    start_spi = true;
    for (block_flash = StartBlock; block_flash < block_flash_max;
         block_flash += Sfc.PageCountInBlock) {
      if (stop)
        break;
  ...
  //////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (block_flash = StartBlock; block_flash < block_flash_max;
         block_flash += NAND_READ_BATCH_PAGES) {
      if (stop)
        break;

      unsigned int nPages = block_flash_max - block_flash;
      if (nPages > NAND_READ_BATCH_PAGES)
        nPages = NAND_READ_BATCH_PAGES;

      FlashDataReadBatch(&flash_xbox[addr_raw], block_flash, nPages,
                         block_size);
    ...
    if (addr_raw) {
      addr_raw = 0;
      int byteWrite;
      for (block_file = StartBlock; block_file < block_flash_max;
           ++block_file) {
        do {
          fseek(fileR, addr_raw, SEEK_SET);
          byteWrite =
              fwrite((char *)&flash_xbox[addr_raw], 1, block_size, fileR);
...
__declspec(dllexport) int emmc_read(const char *file, int startblock,
                                    int length) {
  ...
  const uint32_t BlockCount =
      (length > 0) ? length : (total_blocks - StartBlock);
...
__declspec(dllexport) int emmc_write(const char *file, int startblock) {
  ...
  uint32_t BlockCount =
      (filesize + block_size - 1) / block_size; // Round up if not a multiple
  for (uint32_t i = 0; i < BlockCount; ++i) {
